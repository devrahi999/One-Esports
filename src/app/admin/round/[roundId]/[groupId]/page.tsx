...
import {
  ...
  CheckCircle2, ChevronDown, ChevronUp, Send, Info
} from 'lucide-react';
import { getPlacementPoints, POINT_TABLE } from '@/lib/points';
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        {tab === 'results' && (
          <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-6">
            ...
            {resultRows.length > 0 && (
              <>
                {/* Point reference table toggle */}
                <div className="mb-4">
                  <button
                    onClick={() => setShowPointTable(p => !p)}
                    className="flex items-center gap-2 text-[9px] font-black text-gray-500 uppercase tracking-widest hover:text-white transition-colors"
                  >
                    <Info className="w-3.5 h-3.5" />
                    {showPointTable ? 'Hide' : 'Show'} Point Reference Table
                    {showPointTable ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                  </button>
                  {showPointTable && (
                    <div className="mt-3 grid grid-cols-2 gap-1.5 p-4 bg-black/40 border border-white/5 rounded-xl">
                      {POINT_TABLE.map((row) => (
                        <div key={row.position} className="flex items-center justify-between text-[10px] py-0.5">
                          <span className="text-gray-500">{row.position} {row.label}</span>
                          <span className={`font-black ${row.points > 0 ? 'text-[#ff6a00]' : 'text-gray-600'}`}>
                            {row.points}pts
                          </span>
                        </div>
                      ))}
                      <div className="col-span-2 border-t border-white/5 mt-1 pt-2 text-[10px] text-gray-600">
                        + 1 point per kill
                      </div>
                    </div>
                  )}
                ...
                <div className="space-y-2 mb-5">
                  <div className="grid grid-cols-5 gap-2 text-[9px] font-black text-gray-600 uppercase tracking-widest px-2">
                  ...
                  {resultRows.map((row, idx) => {
                    const placePts = getPlacementPoints(row.position);
                    const killPts = row.kills;
                    const total = placePts + killPts;
                    const isBooyah = row.position === 1;
                    return (
                      <div key={row.teamId} className={`grid grid-cols-5 gap-2 items-center border rounded-xl p-3 transition-colors ${
                        isBooyah ? 'bg-amber-500/10 border-amber-500/20' : 'bg-black/30 border-white/5'
                      }`}>
                        <div className="col-span-2">
                          <p className="text-white font-black text-xs uppercase italic tracking-tight truncate">{row.teamName}</p>
                          {isBooyah && <p className="text-[8px] text-amber-400 font-black uppercase tracking-widest">🏆 Booyah!</p>}
                        </div>
                        <input
                        ...
                        <div className="text-center">
                          <span className="text-[#ff6a00] font-black text-sm">{total}</span>
                          <p className="text-[8px] text-gray-600 font-mono">{placePts}+{killPts}</p>
                        </div>
                      </div>
                    );
                  })}
                </div>
                <p className="text-[9px] text-gray-600 font-black uppercase tracking-widest mb-4">Points = Placement pts + Kill pts (1 per kill) · Tiebreak: Booyahs → Kills → Placement</p>
                <button
            ...
            {leaderboard.length === 0 ? (
              ...
              <>
                <div className="px-6 py-2 bg-black/20 border-b border-white/5">
                  <p className="text-[8px] text-gray-600 font-black uppercase tracking-widest">Tiebreak order: Total Points → Booyahs → Kills → Placement Pts</p>
                </div>
